    ...
    if(isset($_POST['q']) And !empty($_POST['q'])) {
     ...
     ?>         <?php if($profil_autre->rowCount() > 0 && $id_3 != $_SESSION['id']) { ?>
      ...
      <?php while($p = $profil_autre->fetch()) { ?>
        <font color="#684ED9"><b><?= $p['pseudo'] ?> : </b><p class="btn4 msg">Profil</p></font> 
        <?php }
      } else { ?>
...
border-radius: 15px;
    "></div>
        <?php } ?>
        <div class="bio_court" style="font-size: 15px; float: right; text-align: right; font-style: italic; color: #2c3e50;"><?php echo $userinfo['biographie']; ?></div>
       
        <p style="font-size: 20px; float: left; margin-top: 100px; margin-left: -80px; color: #0080C0; font-family: arial; height: 20px;"><b><?php echo $userinfo['pseudo']; ?></b>&nbsp;<br><br><br><br><br>

        <div align="center" class="nav_profil btn11">Mes articles</div>
        <div class="nav_profil btn12">Mes photos</div></p>
        <script type="text/javascript">
  ...
  ?>
        <br>
        <div class="slider2"> <a href="membres/diapo/<?php echo $userinfo['img1'] ?>" target="_blank">
